...
//----------------------------------------------------------------------------------------------
//战斗拓展
var last_use_skill_id = 0;
var S_GP = Game_Battler.prototype;
...
KUR_GAME.prototype._last_Actor_Skill = function () {
    return KUR_GAME.prototype._last_Actor().lastBattleSkill();
};
...
KUR_GAME.prototype._start = function (str, type) {
    var id = 0;
    try {
        switch (type) {
            case "Skill":
                id = KUR_GAME._last_target_Skill_id();
                break;
            case "Actor":
    ...
    switch (str) {
        case "onDamage":
            Effect.prototype.UseSkillonState(id, KUR_GAME.prototype._last_target());
            break;
        case "onDamag":
...
Game_Battler.prototype.onDamage = function (value) {
    ...
    KUR_GAME.prototype._start("onDamage", "Skill");
};
...
Effect.prototype.UseSkillonState = function (skill_id, target) {
    if (skill_id == 0) {
        return null;
    };
    try {
        var st = false;
        var if_ = to_number($dataSkills[skill_id].SZN_if_state.split(' '));
        var add_ = to_number($dataSkills[skill_id].SZN_add_state.split(' '));
